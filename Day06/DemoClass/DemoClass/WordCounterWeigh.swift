...
class WordCounterWeigh: ConsoleScreen {
    ...
    func sortByCount() {
        for var i = 0; i < WordCountNSort.count; ++i
        {
            for var j = WordCountNSort.count - 1; j > 0; --j
            {
                if (WordCountNSort[j].count > WordCountNSort[j-1].count)
                {
                    let temp = WordCountNSort[j]
                    WordCountNSort[j] = WordCountNSort[j-1]
                    WordCountNSort[j-1] = temp
                }
            }
        }
    }

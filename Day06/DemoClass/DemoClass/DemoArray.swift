...
class DemoArray: ConsoleScreen {
    ...
    func declareArray() {
        ...
        //let không ngăn chặn phương thức thay đổi phần tử trong NSMutable....
       ...
       // let index: Int = objMutableArr.indexOfObjectIdenticalTo(Triangle(a: 10, b: 10, c: 10))
        let index: Int = objMutableArr.indexOfObjectIdenticalTo(aTriangle)
        if index == Int.max {
            self.writeln("Cannot find identical triangle")
        } else {
            self.writeln("Found identical object at \(index)")

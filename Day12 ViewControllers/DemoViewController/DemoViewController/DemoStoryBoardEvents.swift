...
class DemoStoryBoardEvents: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.view.addConstraints(NSLayoutConstraint.constraintsWithVisualFormat("V:[button(50)]", options: NSLayoutFormatOptions.allZeros, metrics: nil, views: ["button": button]))
        self.view.addConstraints(NSLayoutConstraint.constraintsWithVisualFormat("H:[button(200)]", options: NSLayoutFormatOptions.allZeros, metrics: nil, views: ["button": button]))
        
        button.addTarget(self, action: "onTap", forControlEvents: UIControlEvents.TouchUpInside)
    }
    
    func onTap() {
        let storyBoard = UIStoryboard(name: "StoryboardEvents", bundle: nil)
